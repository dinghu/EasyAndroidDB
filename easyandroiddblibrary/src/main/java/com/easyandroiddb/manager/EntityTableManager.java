package com.easyandroiddb.manager;
...
import com.easyandroiddb.SQLBuilder;
import com.easyandroiddb.annotation.Column;
import com.easyandroiddb.annotation.NotDBColumn;
import com.easyandroiddb.annotation.PrimaryKey;
import com.easyandroiddb.bean.EntityTable;
import com.easyandroiddb.bean.Property;
import com.easyandroiddb.util.CursorUtil;
import com.easyandroiddb.util.ValueUtil;
...
public final class EntityTableManager implements Serializable {
	...
	public synchronized static <T> void checkOrCreateTable(SQLExecuteManager sqlExecuteManager, T entity) {
		checkOrCreateTable(sqlExecuteManager, entity.getClass());
	}
	...
	public static void checkOrAlterTableColumn(SQLExecuteManager sqlExecuteManager, EntityTable entityTable) {
		...
		List<String> columns = Arrays.asList(cursor.getColumnNames());//数据表中已有的字段信息
		
		List<Property> addColumns = new ArrayList<Property>();
		for(String key : propertys.keySet()) {
			if(!columns.contains(propertys.get(key).getColumn())) {//数据表中不包含当前实体属性字段信息，说明当前实体属性字段需要新增到数据表中
		...
		for(int i = 0; i < fields.length; i++) {
			...
			if(entity.getPrimaryKey() == null) {//主键设置过后即不再遍历主键注解
				...
				if(key != null) {//声明了主键字段
					...
					com.easyandroiddb.bean.PrimaryKey primaryKey = new com.easyandroiddb.bean.PrimaryKey();
